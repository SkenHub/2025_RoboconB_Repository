...
int main(void){
	...
	while(1){
		if(target > kyori_mtr){
		    tar_moku_sa = target - kyori_mtr;
		}
		else if(target < kyori_mtr){
		    tar_moku_sa = kyori_mtr - target;
		}
		for (int i = 0; i < 4; ++i) {
			if(enc_data[i] < 0){
		...
		sken_system.canTransmit(CAN_2, 0x350, send_data_DD,6, 0);
		sken_system.canTransmit(CAN_2, 0x200, send_data,6, 0);
	    if(botan[0] == 1){
	    	if(limitdata[1] == 0)
	    	    mtr[0].write(-30);
	    ...
	    /*if(botan[4] >= 1 && count_takasa < 4){
	    	count_takasa += 1;
	    	botan[4] = 0;

	    }
	    if(botan[5] >= 1 && count_takasa > -1){
	    	count_takasa -= 1;
	    	botan[5] = 0;
	    }
	    else if(count_takasa == 0){
	    	target = 0;
	    }
	    else if(count_takasa == -1){
	    	target = -11;
	    }
	    else if(count_takasa == 1){
	    	target = 31;
	    }
	    else if(count_takasa == 2){
	    	target = 36;
	    }
	    else if(count_takasa == 3){
	    	target = 41;
	    }
	    else if(count_takasa == 4){
	    	target = 60.7;
	    }*/
	    if(botan_og[4] >= 1 && kyori_mtr <= 58.5){
	    	mtr[2].write(-200);
	    }
	    else if(botan_og[5] >= 1 && kyori_mtr >= -15){
	    	mtr[2].write(200);
	    }
	    else{
	    	if(botan[6] == 0)
