...
void can(void){
	if(can_data_r.rx_stdid == 0x340){//canIDが0x201なら実行
		for(int m=0;m<8;m++){
		  c[m] = can_data_r.rx_data[m];
		}
	}
	if(can_data_r.rx_stdid == 0x350){//canIDが0x201なら実行
		for(int m=0;m<8;m++){
	...
	if(can_data_r.rx_stdid == 0x555){//canIDが0x201なら実行
		for(int m=0;m<8;m++){
		  g[m] = can_data_r.rx_data[m];
		}
	}
	if(can_data_r.rx_stdid == 0x556){//canIDが0x201なら実行
		for(int m=0;m<8;m++){
