...
Gpio SW;
Motor mtr[4];
int botan[13],botan_og[13],botan_k[13];
int limitdata[8];
int mtr1_sg[20];
Encoder encoder[4];
...
CanData can_data_r;
uint8_t candata_r[8],can_enc_data[8],botan_tanti;
...
void can_rceive(void){
	if(can_data_r.rx_stdid == 0x400){//canIDが0x400なら実行
		...
		//candata_r[0]の特定のビットが立っているなら、対応するボタンをONにする
	  	if(candata_r[0] && 0x01 == 1){
	  	  botan[0] = 1;
	  	}
	  	else{
	  	  botan[0] = 0;
	  	}
	  	if(candata_r[0] && 0x02 == 2){
	  	  botan[1] = 1;
	  	}
	    else{
	  	  botan[1] = 0;
	  	}
	  	if(candata_r[0] && 0x04 == 4){
	  	  botan[2] = 1;
	  	}
	  	else{
	  	  botan[2] = 0;
	  	}
	  	if(candata_r[0] && 0x08 == 8){
	  	  botan[3] = 1;
	  	}
	  	else{
	  	  botan[3] = 0;
	  	}
	  	if(candata_r[0] && 0x10 == 16){
	  	  botan[4] = 1;
	  	}
	  	else{
	  	  botan[4] = 0;
...
int main(void){
	...
	while(1){
	    ...
	    if(botan[0] == 1){//1押し
			...
			if(enc_sinti[0]<=175 && mtr1_sg[1] == 3){
				mtr[0].write(20);
			}
