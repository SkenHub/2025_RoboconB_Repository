...
bool PS4::Getdata(PS4_data* ps4_data){
	for(int i=0;i<8;i++){
		...
		if(ps4_data->rx_data[i] == 0xa5 && ps4_data->rx_data[(i+1)%8] == 0xa5){
			...
			ps4_data->LxPad    = ps4_data->rx_data[(i+4)%8] - 100;
			ps4_data->LyPad    = ps4_data->rx_data[(i+5)%8] - 100;
			ps4_data->RxPad    = ps4_data->rx_data[(i+6)%8] - 100;
			ps4_data->RyPad    = ps4_data->rx_data[(i+7)%8] - 100;
		}
	}
}
